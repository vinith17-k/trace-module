...
const INITIAL_THRESHOLDS: ThresholdTier[] = [
  {
    ...
    min: 80,
    max: 100,
    actionSummary: "Immediate police intervention, witness safehouse, armed escort",
  },
  {
    tier: "high",
    label: "High Severity",
    min: 60,
    max: 79,
    actionSummary: "Same-day clinical counselling referral and district legal aid cell",
  },
  {
    tier: "moderate",
    ...
    min: 35,
    max: 59,
    actionSummary: "Scheduled counselling follow-up within 48 hours",
  },
  {
    tier: "low",
    ...
    min: 0,
    max: 34,
    actionSummary: "Automated self-help resources and helpline callback on request",
  },
];
  ...
  return (
    <StaffLayout mode="admin">
      <div className="auth-topline">
        <div>
          ...
          <p style={{ fontSize: 12.5, color: "var(--a-muted)", margin: "4px 0 0" }}>
            Map 0–100 SVI score brackets to statutory severity categories and automatic dispatch
            tiers
          </p>
        </div>
      ...
      <p className="inline-legend" style={{ margin: "0 0 16px", display: "inline-block" }}>
        Cases scoring within each bracket automatically trigger the corresponding protocol.
        Threshold changes are logged with administrative accountability.
      </p>
            ...
            <tbody>
              {thresholds.map((row, idx) => (
                <tr key={row.tier}>
                  ...
                  <td style={{ fontSize: 12.5, color: "var(--a-text)" }}>{row.actionSummary}</td>
                </tr>
              ))}
            </tbody>
          </table>
