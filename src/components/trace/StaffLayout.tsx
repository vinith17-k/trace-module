...
export function StaffLayout({ children, mode = "staff", userRole, userName }: Props) {
  const routerState = useRouterState();
  const currentPath = routerState.location.pathname;

  const iconProps = {
    size: 15,
    style: { verticalAlign: "-3px", marginRight: 4 } as React.CSSProperties,
  };
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, color: "var(--a-muted)", textTransform: "capitalize" }}>
                {displayRole}
              </div>
            </div>
          </div>
