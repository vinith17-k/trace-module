...
const PHASES = [
  ...
  { id: 'p4', name: 'Phase 4: Investigation & Charge-sheet', timeframe: '48h - 60 Days', color: '#6FA287' },
  { id: 'p5', name: 'Phase 5: Trial & Rehabilitation', timeframe: '60 - 180 Days', color: '#3FB950' },
];
...
const INITIAL_MILESTONES: MilestoneItem[] = [
  ...
  },
  {
    ...
    checklist: [
      { id: 'c13', text: 'Caste validity certificates seized & verified from Tehsildar', done: true },
  ...
  },
  {
    ...
    checklist: [
      ...
      { id: 'c19', text: 'Remaining 75% rehabilitation & agricultural land allotment', done: false },
...
export const RoadmapTimeline: React.FC<RoadmapTimelineProps> = ({ onNotify }) => {
  ...
  const [newMilestone, setNewMilestone] = useState<Omit<MilestoneItem, 'id' | 'checklist' | 'notes'>>({
    caseRef: 'TR-2026-091',
    title: '',
    agency: 'State Police QRT',
    leadOfficer: '',
  ...
  const filteredMilestones = milestones.filter(m => {
    ...
    const matchesAgency = filterAgency === 'all' || m.agency.toLowerCase().includes(filterAgency.toLowerCase());
  ...
  const handleToggleChecklist = (milestoneId: string, checkId: string) => {
    setMilestones(prev => prev.map(m => {
      if (m.id === milestoneId) {
        const updatedChecklist = m.checklist.map(c => c.id === checkId ? { ...c, done: !c.done } : c);
        ...
        const updatedMilestone = {
          ...
          progressPct: newPct,
          status: newPct === 100 ? ('completed' as const) : m.status === 'not_started' ? ('in_progress' as const) : m.status
        };
  ...
  const handleUpdateMilestone = (field: keyof MilestoneItem, value: any) => {
  ...
  const getStatusBadge = (status: MilestoneItem['status']) => {
    switch (status) {
      case 'completed':
        return <span className="badge badge-low" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><CheckCircle2 size={11} /> Completed</span>;
      case 'in_progress':
        return <span className="badge badge-med" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><Clock size={11} /> In Progress</span>;
      case 'action_needed':
        return <span className="badge badge-high" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><AlertCircle size={11} /> Action Needed</span>;
      case 'blocked':
        return <span className="badge badge-critical" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><ShieldAlert size={11} /> Blocked</span>;
      default:
        return <span className="badge" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>Not Started</span>;
    }
  };

  const getPriorityColor = (priority: MilestoneItem['priority']) => {
    switch (priority) {
      case 'critical': return 'var(--a-critical)';
      case 'high': return 'var(--a-high)';
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: 14 }}>Notion-Style Statutory Case Progression Roadmap</span>
          ...
          <div className="notion-timeline-header">
            ...
            {PHASES.map((p) => (
              <div key={p.id}>
                ...
                <div style={{ fontSize: 10.5, color: 'var(--a-muted)', marginTop: 2 }}>{p.timeframe}</div>
              </div>
            ))}
          </div>
              ...
              <div key={m.id} className="notion-timeline-row">
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                    ...
                    />
                    <span style={{ fontSize: 10.5, fontFamily: 'monospace', color: 'var(--a-muted)' }}>
                      {m.caseRef}
                    </span>
                ...
                <div className="notion-bar-track">
                  ...
                  <div style={{ 
                    ...
                    {PHASES.map((p, i) => (
                      <div key={p.id} style={{ borderRight: '1px dashed rgba(255,255,255,0.05)', height: '100%' }} />
                    ))}
                  </div>
                  ...
                  <div 
                    ...
                    style={{
                      ...
                      border: `1px solid ${
                        m.status === 'completed' ? 'rgba(63,185,80,0.5)' : 
                        m.status === 'blocked' ? 'rgba(224,88,79,0.5)' :
                        m.status === 'action_needed' ? 'rgba(224,162,61,0.5)' : 'var(--a-accent)'
                      }`,
                    }}
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, overflow: 'hidden' }}>
                      <span style={{ fontSize: 11.5, fontWeight: 700, whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                        {m.title}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: 10.5, opacity: 0.75, fontFamily: 'monospace' }}>{m.targetDeadline}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {selectedMilestone && (
        <div style={{
          ...
          <div style={{
            ...
            {/* Modal Header */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '1px solid var(--a-border)', paddingBottom: 16 }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  <span className="badge" style={{ fontFamily: 'monospace' }}>{selectedMilestone.caseRef}</span>
                  <span style={{ color: getPriorityColor(selectedMilestone.priority), fontWeight: 700, fontSize: 11.5, textTransform: 'uppercase' }}>
            ...
            <div style={{ 
              ...
              <span style={{ color: 'var(--a-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <select 
                  ...
                  onChange={(e) => handleUpdateMilestone('startPhaseIndex', parseInt(e.target.value))}
                  style={{ height: 32, fontSize: 12 }}
                >
                  {PHASES.map((p, idx) => (
                    <option key={p.id} value={idx}>Start: {p.name}</option>
                  ))}
                </select>
                <span>to</span>
                <select 
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                />
                <span style={{ fontWeight: 700, minWidth: 40 }}>{selectedMilestone.progressPct}%</span>
              </div>
            </div>
            ...
            <div style={{ padding: '16px 0', borderBottom: '1px solid var(--a-border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', color: 'var(--a-muted)' }}>
                  Statutory Checklist & Verifications ({selectedMilestone.checklist.filter(c => c.done).length}/{selectedMilestone.checklist.length})
                </span>
              </div>
            ...
            <div style={{ padding: '16px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 700, textTransform: 'uppercase', color: 'var(--a-muted)', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxHeight: 180, overflowY: 'auto' }}>
                {selectedMilestone.notes.length === 0 ? (
                  <div style={{ color: 'var(--a-muted)', fontSize: 12, textAlign: 'center', padding: '12px 0' }}>
                    No notes recorded on this milestone yet.
                  </div>
                ) : (
                  selectedMilestone.notes.map(n => (
                    <div key={n.id} style={{ background: 'var(--a-panel2)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 8, padding: '10px 12px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 12, fontWeight: 700, color: '#fff' }}>{n.author}</span>
                          <span className="badge" style={{ fontSize: 9.5 }}>{n.role}</span>
                        </div>
                        <span style={{ fontSize: 10.5, color: 'var(--a-muted)' }}>{n.time}</span>
                      </div>
                      <p style={{ margin: 0, fontSize: 12.5, color: 'var(--a-text)', lineHeight: 1.4 }}>
                        {n.text}
                      </p>
                    </div>
                  ))
            ...
            {/* Footer */}
            <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: 12, borderTop: '1px solid var(--a-border)' }}>
              <button 
                className="btn btn-primary"
                onClick={() => setSelectedMilestone(null)}
              >
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700 }}>Add Statutory Milestone Track</h3>
              </div>
              <button 
                className="btn btn-secondary" 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setNewMilestone({ ...newMilestone, leadOfficer: e.target.value })}
                    required
                  />
                </div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setNewMilestone({ ...newMilestone, startPhaseIndex: parseInt(e.target.value) })}
                  >
                    {PHASES.map((p, idx) => (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <div>
                  ...
                  <select 
                    ...
                    onChange={(e) => setNewMilestone({ ...newMilestone, spanPhases: parseInt(e.target.value) })}
                  >
                    <option value={1}>1 Phase Duration</option>
                    <option value={2}>2 Phases Duration</option>
                    <option value={3}>3 Phases Duration</option>
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setNewMilestone({ ...newMilestone, targetDeadline: e.target.value })}
                  />
                </div>
              </div>
